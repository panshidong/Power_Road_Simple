...
from plot_resilience import plot_triangles_seperate
from plot_resilience import plot_triangle_tot
import random
...
def resilience_triangle(functionality,time):
    ...
    for i in range(len(functionality)):
        complement+=(functionality_for_triangle[i]+functionality_for_triangle[i+1])*time[i]/2
    return -complement
...
def resilience_evaluation(repair_seq):
    ...
    while len(repair_seq)>0:
        ...
        #repair and continue
        time.append(repair_path_time('flows.txt',repair_seq[0])) 
...
def heuristic_find_solution(initial_sequence,consider_interdependence):
    ...
    def process_individual(individual):
        if isinstance(individual, int):
            # 如果是整数，转换为张量并放到 GPU 上
            return torch.tensor(individual, device='cuda')
        elif isinstance(individual, (tuple, list)):
            # 如果是 tuple 或 list，递归处理每个元素，并保留结构
            return type(individual)(process_individual(sub) for sub in individual)
        else:
            raise ValueError(f"Unsupported type: {type(individual)} in individual")

    # 示例的 fitness 计算函数
    def eval_one_max(individual):
        # 递归处理 individual，将其转换为合适的张量结构
        processed_individual = process_individual(individual)
        
        # 这里你可以执行你的计算逻辑，比如调用 resilience_evaluation
        # 假设 resilience_evaluation 可以处理嵌套的张量结构
        fitness = resilience_evaluation(processed_individual)[0]
        
        print(f"Processed Individual: {processed_individual}, Fitness: {fitness}")  # 调试输出
        return (fitness.item(),)  # 返回适合 DEAP 的格式
...
with open(result_folder+'output.txt', 'w') as f:
    ...
    print("total resilience: ", result_opt, file=f)
